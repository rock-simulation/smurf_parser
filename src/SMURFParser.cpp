...
namespace smurf_parser {
  ...
  boost::filesystem::path resolvePath( const boost::filesystem::path& p, const boost::filesystem::path& base) {
    ...
    for(boost::filesystem::path::iterator it=abs_p.begin(); it!=abs_p.end(); ++it) {
      if (*it == "..") {
        ...
        result /= *it; // Just cat other path entries
    }
    return result;
  }

  void getAbsoluteFilePaths(std::string path, configmaps::ConfigMap* map, std::string smurffilename,
    bool expandURIs, std::string &absolute_urdf_path, std::string &absolute_srdf_path,
    ...
    for(it = (*map)["files"].begin(); it!=(*map)["files"].end(); ++it) {
      boost::filesystem::path filepath((std::string)(*it));
      if (filepath.extension().generic_string() == ".urdf") {
        abs_path_of_file = resolvePath(filepath.generic_string(), path) ;
        absolute_urdf_path = abs_path_of_file.string();
      }
      if (filepath.extension().generic_string() == ".srdf") {
        abs_path_of_file = resolvePath(filepath.generic_string(), path) ;
        absolute_srdf_path = abs_path_of_file.string();
      }
      else if (filepath.extension() == ".yml") {
        abs_path_of_file = resolvePath(filepath.generic_string(), path) ;
